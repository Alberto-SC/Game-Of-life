class Universe{
    ...
    setCell(x,y,alive){
		...
		// console.log("HI");
		this.root = this.node_setCell(this.root,x,y,alive);
	}
	node_setCell(node,x,y,alive){
		if(node.level ===0)
			return alive?this.true_leaf:this.false_leaf;
		...
		if(x<0){
			if(y<0){
				...
				nw = this.node_setCell(nw,x+ofset,y+ofset,alive);
			}
			else {
				// console.log("Hello sw")
				sw = this.node_setCell(sw,x+ofset,y-ofset,alive);
			}
		}
		else{
			if(y<0){
				// console.log("HELLO ne")	
				ne = this.node_setCell(ne,x-ofset,y+ofset,alive);
			}
			else{
				// console.log("HELLO se")	
				se = this.node_setCell(se,x-ofset,y-ofset,alive);
			}
		}
		// console.log(nw,ne);
		return this.create_tree(nw,ne,sw,se);
